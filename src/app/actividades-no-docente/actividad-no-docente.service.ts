...
export class ActividadNoDocenteService {
  ...
  constructor(private http: HttpClient) { }
    ...
    private urlEndPoint:string = 'http://localhost:8080/api/actividades-no-docentes/listarActividadesNoDocentes'

    private urlEndPoint_1:string = 'http://localhost:8080/api/actividades-no-docentes/guardarActividadNoDocente'
 ...
 private urlDocente1: string = 'http://localhost:8080/api/docente/listar';
    ...
    private httpHeaders = new HttpHeaders({ 'Content-Type': 'application/json'});
      ...
      create(asignatura: ActividadesNoDocente): Observable<ActividadesNoDocente> {
        return this.http.post<ActividadesNoDocente>(this.urlEndPoint_1, asignatura, { headers: this.httpHeaders })
      }
 ...
 updateAsignatura(actN: ActividadesNoDocente): Observable<ActividadesNoDocente> {
...
getActNoDocenteByName(nombre: string): Observable<ActividadesNoDocente> {
  return this.http.get<ActividadesNoDocente[]>(this.urlEndPoint).pipe(
    map(actNo => actNo.find(actNoDoc => actNoDoc.actiNoDoc_nombre === nombre) as ActividadesNoDocente),
